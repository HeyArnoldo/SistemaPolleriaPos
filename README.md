...
| `TOTP_CHALLENGE_EXPIRES`            | Tiempo de vida del token de desafío 2FA (paso intermedio entre ingresar contraseña y confirmar código TOTP). El token es un JWT de corta duración firmado con `JWT_SECRET` que autoriza únicamente completar el segundo paso del login. Si el usuario tarda más de este tiempo en ingresar su código, deberá iniciar sesión nuevamente. Formato: número de segundos (entero) o sufijo `d`/`h`/`m`/`s` (ej. `5m`, `300`). Predeterminado: `5m`. No configurable en la mayoría de los casos; aumentar solo si los usuarios tienen demoras frecuentes entre pasos.                    | `5m`                        |
